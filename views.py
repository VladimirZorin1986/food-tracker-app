...
from app import app
from common_funcs import convert_date
...
def index():
    ...
    if request.method == 'POST':
        ...
        db.commit()
    cur = db.execute('''select log_date.entry_date as entry_date, 
    ...
    date_results = [{'pretty_date': convert_date(item), 'entry_date': item['entry_date'],
                     'protein': item['protein'], 'carbohydrates': item['carbohydrates'],
                     'fat': item['fat'], 'calories': item['calories']} for item in results]
...
def view(date):
    ...
    result = cur.fetchone()
    pretty_date = convert_date(result)
    ...
    totals = {}
    for attr in ('protein', 'carbohydrates', 'fat', 'calories'):
        totals[attr] = sum(x[attr] for x in log_results)

    return render_template('day.html', entry_date=result['entry_date'], pretty_date=pretty_date, food_results=food_results,
